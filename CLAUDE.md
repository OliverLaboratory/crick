...
- `crick/corpus.py` — where epoch instances come from. `SyntheticCorpus` (seed-generated, the prototype/test mode) and `ManifestCorpus` (**real, content-addressed data**: genesis commits a manifest hash; each fetched blob is checked against its sha256, so the data mirror can be untrusted). `corpus_from_genesis(spec)` dispatches on whether the genesis spec has a `manifest_url`. With real data, determinism rests only on the integer verifier — no shared PRNG.
- `tools/build_corpus.py` — offline pipeline: real PDB structures → integer Cα contact graphs → content-hashed blobs + `manifest.json`. Float distance math is offline/one-time; committed data is pure integers. Not part of the consensus package.
- `crick/chain.py` — the heart: validation, account-model state (balances + per-sender nonces), and the DIPS v2 independent retargeting (`_update_schedule` / `_retarget_db` / `_retarget_dr` / `_drought_discount`). Holds a `corpus` object; `_instance(i)` delegates to it. Chains received from peers are fully re-validated via `from_block_dicts` (which rebuilds and hash-verifies the corpus); fork choice is total accumulated work (`adopt_if_better`).
